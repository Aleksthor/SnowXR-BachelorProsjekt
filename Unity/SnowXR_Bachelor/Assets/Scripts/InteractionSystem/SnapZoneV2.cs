...
namespace BNG
{
    public class SnapZoneV2 : MonoBehaviour
    {
        ...
        /// Multiply Item Scale times this when in snap zone.
        /// </summary>
        [Tooltip("If the grabber is too far apart form the snap zone when grabbing the object, the snapped object will teleport to the grabber.")]
            ...
            if (grabber != null)
            {
                if (HeldItem)
                {
                    ...
                    if (DuplicateItemOnGrab)
                    {
                        ...
                        if (Vector3.Distance(g.transform.position, grabber.transform.position) <= SnapDistance)
                        {
