...
// Taken from https://gist.github.com/mickdekkers/5c3c62539c057010d4497f9865060e20
...
public class SnapshotCamera : MonoBehaviour {
    ...
    {
        return TakeSnapshot(this.cam, backgroundColor, width, height);;;;
    }
    /// <summary>
    ...
    /// <param name="height">The height of the snapshot image.</param>
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    public Texture2D TakeSnapshot(Camera camBehaviour, Color backgroundColor, int width, int height)
    {
        // Set the background color of the camera
        camBehaviour.backgroundColor = backgroundColor;

        // Get a temporary render texture and render the camera
        camBehaviour.targetTexture = RenderTexture.GetTemporary(width, height, 24);
        camBehaviour.Render();

        // Activate the temporary render texture
        RenderTexture previouslyActiveRenderTexture = RenderTexture.active;
        RenderTexture.active = camBehaviour.targetTexture;

        // Extract the image into a new texture without mipmaps
        Texture2D texture = new Texture2D(camBehaviour.targetTexture.width, camBehaviour.targetTexture.height, TextureFormat.ARGB32, false);
        texture.ReadPixels(new Rect(0, 0, camBehaviour.targetTexture.width, camBehaviour.targetTexture.height), 0, 0);
        texture.Apply(false);
        ...
        camBehaviour.targetTexture = null;
        RenderTexture.ReleaseTemporary(camBehaviour.targetTexture);
