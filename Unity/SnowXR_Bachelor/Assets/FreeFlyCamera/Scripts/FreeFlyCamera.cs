...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    private float _overrideSpeed = 5f;
    private void Update()
    {
        if (!_active)
            return;
        ...
        if (_enableMovement)
        {
            ...
            _overrideSpeed += Input.mouseScrollDelta.y * 0.2f;
            _overrideSpeed = Mathf.Clamp(_overrideSpeed, 0.001f, 20f);
            ...
            transform.position += deltaPosition * (_overrideSpeed + currentSpeed) * _currentIncrease;
            // transform.position += deltaPosition * _overrideSpeed;
        }
