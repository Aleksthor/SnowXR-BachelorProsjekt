    ...
    public class HandController : MonoBehaviour {
        ...
        public virtual void UpdateFromInputs() {
            ...
            if (grabber == null || !grabber.isActiveAndEnabled) {
                ...
                PointAmount = 0;
                ThumbAmount = 0;
                Debug.LogError("Missing grabber");
                return;
            }
            ...
            if (input.SupportsIndexTouch && _indexIsNear == false && PointAmount != 0) {
                PointAmount = 1f;
                //Debug.Log($"2 {input.SupportsIndexTouch} && {_indexIsNear} == false && {PointAmount} != 0");
            }
            // Does not support touch, stick finger out as if pointing if no trigger found
                ...
                PointAmount = 1;
                //Debug.Log($"1 !{input.SupportsIndexTouch} && {_triggerValue} == 0");
            }
            //Debug.Log($"Point amount for {grabber.HandSide} is now {PointAmount}", this);
        }
